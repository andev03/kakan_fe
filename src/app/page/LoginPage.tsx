...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (orderId) {
      console.log("Order ID nhận từ URL:", orderId);
      toast.success("Bạn đã đăng ký gói Premium. Hãy đăng nhập để sử dụng.");
      // Bạn có thể lưu orderId vào state, gọi API, hoặc hiển thị thông báo
    }
  }, [orderId]);
  ...
  useEffect(() => {
    if (location.state?.toastMessage) {
      toast.error(
        "Tài khoản của bạn đã khóa . Vui lòng liên hệ quản trị viên."
      );
    }
  }, [location.state]);
